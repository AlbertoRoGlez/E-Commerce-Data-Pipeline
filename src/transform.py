...
def query_freight_value_weight_relationship(database: Engine) -> QueryResult:
    ...
    data = items.merge(orders, on='order_id', how='left').merge(products, on='product_id', how='left')
    ...
    aggregations = aggregations.astype({'freight_value': float, 'product_weight_g': int})
    aggregations = aggregations.reset_index()


    #aggregations = aggregations["freight_value"] = pd.to_numeric(aggregations["a"])
...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    order_purchase_amount_per_date = filtered_dates.groupby(by="order_purchase_timestamp")["order_id"].count().reset_index()
    order_purchase_amount_per_date.rename(columns={"order_id": "order_count"}, inplace=True)
    ...
    result_df = pd.merge(order_purchase_amount_per_date, holidays, left_on="order_purchase_timestamp", right_on="date", how="left")
    result_df["holiday"] = result_df["localName"].notna()
    result_df = result_df[["order_count", "order_purchase_timestamp", "holiday"]]
